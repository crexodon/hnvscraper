...
select_query = """SELECT latitude, longitude, bus_timestamp, current_stop, next_stop, realtime_available, current_delay, bus_name, bus_journey, get_timestamp 
                      FROM vehicles 
                      WHERE get_timestamp = (SELECT MAX(get_timestamp) FROM vehicles WHERE line_id = (?));"""
lines = ["hnv:30001:E:R:j25", "hnv:30001:E:H:j25", "hnv:100S4: :H:j25", "hnv:100S4: :R:j25"]
for line in lines:
